...
import thejester from "@/assets/cards/thejester.png";
...
export function TheJester({ name, messagesSent }: TheJesterProps) {
  ...
  useEffect(() => {
    ...
    async function renderCanvas() {
      ...
      ctx.textAlign = "center";
      ctx.fillText(
        "Laughing Emojis Sent: " + messagesSent.toString(),
        canvas.width / 2,
        1200,
      );
      ...
      // Text to display
      const text =
        "The laugh-track of the chat. Drops 😂 and 💀 on the slightest joke, turning even grocery lists into comedy sketches. When the convo quiets, one perfectly timed meme sets everyone wheezing again.";
